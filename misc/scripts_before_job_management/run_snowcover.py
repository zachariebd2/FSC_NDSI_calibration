...
from sc_preprocess import *
from sc_utils import *
from sc_analyse_datasets import *
...
def process_data():
    ...
    for s in data["sets"]:
        ...
        if s["plt_periode"] :
            ...
            out = s["out"]
            analyse_period(p,out)
        if s["quicklooks"] :
            print("making quicklooks for "+s["out"])
            out = s["out"]
            make_quicklooks(p,out)
        ...
        if c["eval_target"] :
            ...
            for e in data["sets"]:
                ...
                if e["eval_cal"] and (out_cal != out_val):
                    ...
                    eval_res = e["eval_res"]
                    evaluation(p,out_cal,out_val,eval_modes,eval_res)
            ...
            for DP in data["DP"]:
                if DP["eval_cal"] :
                    ...
                    print("evaluating "+out_cal+" with "+out_val)
                    if snw_type == "fsc": evalFSCWithDP(out_cal,out_val,p)
                    if snw_type == "sca": evalSCAWithDP(out_cal,out_val,p)
...
def process_params():
    ...
    if end_date == "": 
        end_date = start_date
    if getDateFromStr(start_date) == '' or getDateFromStr(end_date) == '':
    ...
    for s in inputs["sets"]:
        ...
        print("    Make FSC/NDSI datasets : ", s["make_set"])
    ...
    for DP in inputs["DP"]:
        ...
        print("    Create list of valid data points : ", DP["do_filter"])
