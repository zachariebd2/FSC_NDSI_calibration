...
def isCoordInside(Gbig,x,y,cEPSG="4326"):
...
def getCM2Dmetrics(test,pred,P=1,N=0):
    #kappa
    ...
    for i in range(len(pred)): 
        if test[i]==pred[i]==P:
            TP += 1.0
        if pred[i]==P and test[i]!=pred[i]:
            FP += 1.0
        if test[i]==pred[i]==N:
            TN += 1.0
        if pred[i]==N and test[i]!=pred[i]:
            FN += 1.0

    total = TP + FP + TN + FN


    return kappa,acc,TP, FP, TN, FN, total
...
def plot_confusion_matrix(cm,target_names,cmap=None):
    ...
    if target_names is not None:
        ...
        plt.yticks(tick_marks, target_names,verticalalignment="center",rotation=90)


    cm_per = cm.astype('float') / cm.sum() * 100


    thresh = cm.max() / 2
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):

        plt.text(j, i, "{}\n\n{:.2f} %".format(int(cm[i, j]),cm_per[i, j]),
